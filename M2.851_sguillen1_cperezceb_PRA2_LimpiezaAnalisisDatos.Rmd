...
Empezamos haciendo un breve revisión de los datasets; número de registros, tipos de datos, distribuciones de las variables..

Para ello cargamos los datos de los ficheros en la variable _data_.
Primero leeremos el fichero train.csv
...
Completamos el dataset de Test con la variable _Survived_, por lo que, hacemos un merge con los otros ficheros, test y gender_submission. 
Verifición del tamaño de nuestros conjuntos de datos y sus variables.
...
y finalmente unificamos nuestros dataset en un único dataframe, _data_
...
Verificamos la consistencia de nuestro dataset. 

Comprobamos si existen duplicados, basándonos en el identificador del pasajero.

```{r}
#Obtener los valores repetidos para la variable PassengerId
duplicados <- unique(data[duplicated(data$PassengerId, fromLast=TRUE),])
table(duplicados$PassengerId)
```
...
Nuestra muestra contiene un total de 1309 registros y 12 variables distintas; 5 de tipo carácter, 5 de tipo integer y 2 numéricas.
...
Basándonos en las métricas anteriores, ya podemos extraer algunas conclusiones:

* Hay un pequeño porcentaje de supervivientes.
* La mayoría de los pasajeros viajan en tercera clase.
* En general son personas adultas entre 21 y 39 años pero hay muchos valores desconocidos
* ¿Viajan solos?
...
En caso de haberlos, habría que tomar decisiones para tratarlos adecuadamente y en aquellos casos que el porcentaje sea muy elevado deberemos descartar la variable. De igual manera, también eliminaríamos las observaciones con un número alto de valores vacíos.
...
Observamos que debemos tratar las variables _Age_, _Fare_, _Cabin_ y _Embarked_.

La variable _Cabin_ la descartamos ya que tiene un porcentaje alto de datos vacios, de las 1309 observaciones disponibles 1014 no disponemos el datos.

```{r}
data$Cabin<-NULL
```

Hay dos pasajeros de los cuales no consta el puerto de partida, variable Embarked vacía. Para este caso, les asignamos el origen más repetido, que como hemos comentado es Southampton
...
***
**nota:** `tratar los valores de las variables _Fare_ y _Age_ por los valores próximos a sus vecinos`


## Valores atípicos {.tabset}

Examinemos las variables en busca de valores atípicos. 

Nos ayudaremos de una representación gráfica como puede ser *boxplot*, cada variable será diferente pero consideraremos atípico aquellos valores que se encuentren a 1.5 veces de uno de los cuartiles.

### Pclass {-}

```{r}
boxplot(data$Pclass, col = "red")
```

### Age {-}

```{r}
boxplot(data$Age, col = "palegreen")
```

### SibSp {-}

```{r}
boxplot(data$SibSp, col = "blue")
```

```{r}
 primer_cuartil<-quantile(data$SibSp, prob = 0.25, na.rm = TRUE)
 tercer_cuartil<-quantile(data$SibSp, prob = 0.75, na.rm = TRUE)
 rango_intercuartilico<-IQR(data$SibSp)

 menoresDe <- primer_cuartil - (1.5*rango_intercuartilico)
 mayoresDe <- tercer_cuartil + (1.5*rango_intercuartilico)

 sprintf("El rango de valores que limitan el rango es: %f - %f", menoresDe, mayoresDe )
```

### Parch {-}

```{r}
boxplot(data$Parch, col = "orange")
```

### Fare {-}

```{r}
boxplot(data$Fare, col = "orange")
```


## Normalización, categorización y selección de variables
...
Categorizamos las variables numéricas, _Age_ y _Fare_, en rangos/tramos de valores

**nota:** `igual debemos normalizarlos: data$Age<-(data$Age - min(data$Age)) / (max(data$Age) - min(data$Age))`
...
Tras las transformaciones realizadas, el conjunto de datos presenta el siguiente aspecto:
...
grid.newpage()
  ...
  plotbyEmbarked<-ggplot(data,aes(Embarked))+geom_bar(fill = "#377EB8") +labs(x="", y="Pasajeros")+ggtitle("Origen")
  
grid.arrange(plotbyClass, plotbySex, plotbyEmbarked, ncol=3)
...
**nota::** `revisar los datos porque cambian al aplicar las transformaciones de valores vacíos...`
